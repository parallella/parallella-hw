...
class comp():
    ...
    def getSortRef(self):
        m = re.match( "([a-zA-Z]+)([0-9]+)", self.getRef())
	if m == None:
           return self.GetRef()
        return "%s%06d" % (m.group(1), int(m.group(2)) )
...
class netlist():
    ...
    def groupComponents(self, components = None):
        ...
        for g in groups:
            g.sort(key=lambda g: g.getSortRef())

        # Finally, sort the groups to order the references alphabetically
        groups.sort(key=lambda group: group[0].getSortRef())
